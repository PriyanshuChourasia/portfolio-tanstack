      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {pricing.map((plan: any, i: number) => (
            <div key={i} className="border border-border hover:border-primary/50 rounded-lg p-4 sm:p-6 md:p-8 bg-white dark:bg-linear-to-br dark:from-slate-800 dark:to-slate-900 transition-all hover-lift">
              ...
              <ul className="space-y-3 sm:space-y-4">
                {plan.features.map((feature: any, j: number) => (
                  <li key={j} className="flex items-center gap-2 sm:gap-3">
                    ...
                    {feature.isNew && <span className="bg-linear-to-r from-cyan-500 to-blue-500 text-white dark:text-slate-900 text-[10px] sm:text-xs px-2 py-1 rounded font-semibold">new</span>}
                  </li>
                ))}
              </ul>
            </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 divide-x divide-border md:divide-x">
          {funFacts.map((fact: any, i: number) => (
            <div key={i} className="text-center py-4 sm:py-6 md:py-12 px-2 sm:px-4 hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors rounded">
